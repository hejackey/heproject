...
public class UpdateIndexTask {
	// 获取全部索引存放的位置.
	private String indexPath = PropertyConfiguration.getAllIndexPath();
	// 初始化一个IndexWriter.
	...
	AllDataService dataService = null;
	...
	public synchronized boolean indexUpdate(int pid) {
		...
		dataService = new AllDataService();
		Document doc = dataService.getAPhoneData(pid);
		boolean flag = false;
		
		try {
			// 删除Document
			reader = IndexReader.open(indexPath);
			Term term = new Term("phoneId", pid + "");
	...
	public synchronized void indexUpdate() {
		...
		// 获得上架销售而未索引的手机数量Num
		dataService = new AllDataService();
		int Num = dataService.getUnIndexedPhoneNums();
		...
		try {
			...
			AllIndexser indexser = new AllIndexser(reader);
			for (int i = 0; i < Num; i = i + MAX_UPDATE_DOCS) {
				// 每次返回2万个Document.
				listdoc = dataService.getUnindexedPhoneDatas(i, i
						+ MAX_UPDATE_DOCS);
			...
			e.printStackTrace();
			IndexLogs.setLogs("delete of update is exception!", this.getClass().getName());
		...
		try {
			...
			// 初始化一个PhoneIndexser对象.
			AllIndexser indexser = new AllIndexser(writer);
			// 重新添加Document列表
			for (int i = 0; i < Num; i = i + MAX_UPDATE_DOCS) {
				// 每次返回2万个Document.
				listdoc = dataService.getUnindexedPhoneDatas(i, i
	...
	public static void main(String[] args) {
		UpdateIndexTask ts = new UpdateIndexTask();
		ts.indexUpdate();
	}
}
