...
/**
 * 手机索引文件的创建. 对上架销售的手机全建索引文件(相对于数据库).
 * 
 * @author liuxueyong
 */
public class PhoneCreateIndexTask {
	...
	public void createIndex() {
		...
		try {
			...
			// 获得上架销售的手机数量upNum
			int upNum = phoneDataService.getAllPhoneNums();
			...
			if( upNum != 0 ) {
				...
				for (int i = 0; i < upNum; i = i + MAX_UPDATE_DOCS) {
					// 第次追加2万个Document.
					listdoc = phoneDataService.getAllPhoneDatas(i, i
							+ MAX_UPDATE_DOCS);
