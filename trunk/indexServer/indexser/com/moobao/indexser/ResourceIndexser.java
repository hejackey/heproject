 ...
 * @author liuxueyong
 */
public class ResourceIndexser {
	
	IndexWriter writer = null;
	IndexReader reader = null;
	
	//得到写索引和修改索引对象
	public ResourceIndexser( IndexWriter writer ) {
		this.writer = writer;
		//this.writer.setUseCompoundFile( true );
	}
	
	//得到修改索引对象
	public ResourceIndexser( IndexReader reader) {
		this.reader = reader;
	}
...
//    		//删除Document列表
//    		for( int i = 0; i < listdoc.size(); i ++ ) {
//    			articleID = listdoc.get(i).get( "articleID" );
//    			term = new Term( "articleID", articleID );
//    			reader.deleteDocuments( term );
//    		}
//    		reader.close();
//    		//优化
...
//    }
    ...
    public void indexDel(String keyid) {
    	try {
    		//删除Document
    		Term term = new Term( "articleID", keyid );
        	reader.deleteDocuments(term);
    	}
    	catch( IOException e ) {
    		e.printStackTrace();
