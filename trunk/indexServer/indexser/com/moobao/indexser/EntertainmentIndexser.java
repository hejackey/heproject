 ...
 * @author liuxueyong
 */
public class EntertainmentIndexser {
	
	IndexWriter writer = null;
	IndexReader reader = null;
	
	//得到写索引和修改索引对象
	public EntertainmentIndexser( IndexWriter writer ) {
		this.writer = writer;
	}
	
	//得到修改索引对象
	public EntertainmentIndexser( IndexReader reader) {
		this.reader = reader;
	}
    ...
    public void indexDel(List<Document> listdoc) {
    	...
    	try {
    		//删除Document列表
    		for( int i = 0; i < listdoc.size(); i ++ ) {
    			entID = listdoc.get(i).get( "entID" );
    			term = new Term( "entID", entID );
    			reader.deleteDocuments( term );
    		}
    	}
    	catch( IOException e ) {
    ...
    public void indexDel(String keyid) {
    	try {
    		//删除Document
    		Term term = new Term( "entID", keyid );
        	reader.deleteDocuments(term);
    	}
    	catch( IOException e ) {
    		e.printStackTrace();
