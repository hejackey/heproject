package com.moobao.service.all;
...
import com.moobao.indexser.document.AllDocument;
import com.moobao.service.DataBaseService;

public class AllDataService extends DataBaseService {
	...
	public List<Document> getUnSalePhonePhoneDatas(int beginNum, int endNum) {
		...
		if (endNum >= beginNum && endNum > 0) {
			...
			sql
					.append(" f_get_product_pic(t.productid, 1) photo_name from product_list t ");
			sql
					.append(" join product_mobile_attr pma on t.productid = pma.productid ");

			sql
					.append(" where t.productclass = 1 and t.ifuse = 0  and t.comp_id = 1 ");
			sql
					.append(" or t.productclass = 1 and t.down_time < sysdate and t.comp_id = 1 ");
			sql
					.append(" or t.productclass = 1 and t.ifvalid = 0 and t.comp_id = 1 ");
	...
	public int getAllPhoneNums() {
		...
		sql.append(" where  ");
		sql.append(" n.ifvalid = 1 and n.ifgift = 0 ");
...
//					.append(" (select tt.price_market from product_list tt where tt.ifgoogle = 1 and tt.pri_productid = pmaster.pri_productid) price_market, ");
			sql
					.append(" (select min(t.price_net) from product_list t where pmaster.pri_productid=t.pri_productid ) minNetPrice, ");
			sql
					.append(" (select max(t.price_net) from product_list t where pmaster.pri_productid=t.pri_productid ) maxNetPrice, ");
			sql
					.append(" (select count(*) from user_message m where m.productid = pmaster.pri_productid and m.message_type = 1) recommendCount, ");
			sql
					.append(" (select count(*) from user_message m where m.productid = pmaster.pri_productid and m.message_type = 2) askCount,  ");
			sql.append(" f_get_product_pic(productid, 1) photo_name ");
			sql.append(" from product_master pmaster ");
			sql
					.append(" join (select m.* from( select t.*,row_number() over(partition by t.pri_productid order by t.ifgoogle desc) as rn from product_list t ) m where m.rn=1) n on pmaster.pri_productid = n.pri_productid ");
			sql.append(" where  ");
...
//			sql.append(" and plist.ifgoogle = 1 ");
	...
	private Document buildPhoneIndexDocument(Map entry) {
		...
		if (entry != null) {
			...
			String sql2 = "select * from product_list t where t.pri_productid = ?";
...
//							}
			...
			String productModel = StringUtil
					.escapeNullObject(RowSetTypeConverter.getString(entry
							.get("productmodel")));
			String product_para = StringUtil
					.escapeNullObject(RowSetTypeConverter.getString(entry
							.get("product_para")));
...
//					brandv.intValue(), ifuse.intValue());
			return AllDocument.getDocument_new(pri_productid, classcode, brandv.intValue(), brand_name, productModel, product_para,
					...
					colorStr, ifuse.intValue(), recommendCount.intValue(), askCount.intValue(), photo_name, shape);
		} else {
			return null;
		}
	}
	...
	public int setAPhoneIndexed(int phoneid) {
		if (phoneid > 0) {
			...
			//sql.append(" and t.classcode like '001%' ");
	...
	public static void main(String arg[]) {
		AllDataService peijianDataService = new AllDataService();
