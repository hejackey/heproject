function queryProvider(){
	...
	window.open("../mis/provider/qByNameProvider.do?providerName="+encodeURIComponent(pname),"_blank");
}
...
function showDiaoBoDetail(id,sheetid){
	if($("#zk"+id).html()=="+展开"&& $("#zk"+id).attr('title')=="1"){	
		...
		$.get("getDetailBySheetIdDiaoBo.do?id="+id,function(data){
			if(data!=null && data!=""){
...
function preAddDiaoBo(type){
	document.form1.action="preAddDiaoBo.do?stockopentype="+type;
	document.form1.submit();
}
...
function preEditDiaoBo(id){
	document.form1.action="preEditDiaoBo.do?id="+id;
	document.form1.submit();
}

function editDiaoBo(){
	document.form1.action="editDiaoBo.do";
	document.form1.submit();
}
