...
<div id="rightcontent">
	<div id="breadcrumb">
    	您所在的位置：<a href="#" class="green">首页${model.sheetstate}</a> > <a href="listStockOrder.do" class="green">采购单列表</a>  > <span class="fb black">采购单列表</span>
    </div>    
    <form name="form1" action="listStockOrder.do" method="post">       
    <div id="container">
     <div class="addsell">
   	  <div class="left"><input type="button" onclick="preAddStockOrder(1)" class="btn120x28" style="cursor:pointer" id="addsheet" name="addsheet" value="新增采购单">   	  
   	  <!--<input type="button" onclick="preAddStockOrder(2)" class="btn120x28" style="cursor:pointer" id="addthsheet" name="addthsheet" value="新增退货单">-->
   	  <input type="button" onclick="exportToExcel(1,'aid','updateStatusStockOrder')" class="btn120x28" style="cursor:pointer" id="checksheet" name="checksheet" value="导出Excel">
   	  <#if model.sheetstate?if_exists!=1>   	  
   	  <input type="button" onclick="updateState(1,'aid','updateStatusStockOrder')" class="btn70x28" style="cursor:pointer" id="checksheet" name="checksheet" value="审核单据">   	  
   	  <input type="button" onclick="updateState(2,'aid','updateStatusStockOrder')" class="btn70x28" style="cursor:pointer" id="delsheet" name="delsheet" value="删除">
   	  </#if>
    ...
    <!--上边功能区开始-->
   	  <div class="tooltop nowarp">
   	    <div class="left ">
        	...
        	<input type="button" class="btn48x22" style="cursor:pointer" onclick="queryStockOrder()" id="querystockorder" value="查询"> 	
        </div>        
      </div>
    ...
    <div class="selllist fl_left" style="width:100%">
    	<table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <#list model.stockOrderList?if_exists as list>
          <tr>
            <td class="tablist">
            <input type="checkbox" title="全选/取消全选" name="aid" id="aid" value="${list.id?if_exists}"/>
            ${list_index?if_exists+1}&nbsp;&nbsp;<span id="zk${list.id?if_exists?html}" style="cursor:pointer" onclick="showStockOrderDetail('${list.id?if_exists?html}','${list.sheetid?if_exists?html}')" title="1">+展开</span></td>
            <td class="tablist"><a href="#" onclick="preEditStockOrder('${list.id?if_exists?html}')" class="green">${list.sheetid?if_exists?html}</a></td>
