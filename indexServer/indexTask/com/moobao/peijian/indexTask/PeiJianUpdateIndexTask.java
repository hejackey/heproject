 ...
 * @author liuxueyong
 */
public class PeiJianUpdateIndexTask {
	...
	public synchronized boolean indexUpdate(int pid) {
		...
		Document doc = peiJianDataService.getAllPeiJianDatas_new(pid);
		boolean flag = false;
	...
	public synchronized void indexUpdate() {
		// 获得上架销售而未索引的配件数量Num
		PeijianDataService peiJianDataService = null;
		peiJianDataService = new PeijianDataService();
		...
		try {
			...
			for (int i = 0; i < Num; i = i + MAX_UPDATE_DOCS) {
				listdoc = peiJianDataService.getUnindexedPeiJianDatas_new(i, i
						+ MAX_UPDATE_DOCS);
	...
	public static void main(String[] args) {
		PeiJianUpdateIndexTask ts = new PeiJianUpdateIndexTask();
		ts.indexUpdate();
	}
}
