...
/**
 * 上网本和上网卡索引文件的创建. 对上架销售的上网本和上网卡全建索引文件(相对于数据库).
 * 
 * @author liuxueyong
 */
public class NetComputerCreateIndexTask {
	// 获取上网本和上网卡索引存放的位置.
	...
	public void createIndex() {
		...
		try {			
			...
			netComputerIndexser = new NetComputerIndexser(writer);
			// 获得上架销售的上网本和上网卡数量upNum
			int upNum = netComputerDataService.getAllNetComputerNums();
            ...
            if( upNum != 0 ) {
    			...
    			for (int i = 0; i < upNum; i = i + MAX_UPDATE_DOCS) {
    		        ...
    		        // 第次追加2万个Document.
    				listdoc = netComputerDataService.getAllNetComputerDatas_new(i, i + MAX_UPDATE_DOCS);
    			...
    			// 更新数据库的标志位
    			// 更新所有上架销售上网本和上网卡为已被索引状态
    			netComputerDataService.setUnIndexedNetComputerIndexed();
    			
    			//日志
    			IndexLogs.setLogs("netComputer index is reCreated!", this.getClass().getName());
            }
            else{
				System.out.println("没有记录, 因此没创建上网本和上网卡索引!");
			}
		...
		try {
			...
			System.out.println( "重启上网本和上网卡索引异常!" );
		}
	}

	public static void main(String[] args) {
		NetComputerCreateIndexTask ts = new NetComputerCreateIndexTask();
		ts.createIndex();
	}
}
