 ...
 * @author liuxueyong
 */
public class PeijianBaseCreateIndexTask {
	// 获取配件索引存放的位置.
	...
	public void createIndex() {
		...
		try {			
            ...
            if( upNum != 0 ) {
    			...
    			for (int i = 0; i < upNum; i = i + MAX_UPDATE_DOCS) {
    		        ...
    		        // 第次追加2万个Document.
    				listdoc = peiJianDataService.getAllPeiJianDatas(i, i + MAX_UPDATE_DOCS);
    			...
    			//日志
    			IndexLogs.setLogs("peiJianBASE index is reCreated!", this.getClass().getName());
            }
            else{
		...
		try {
			...
			IndexLogs.setLogs("peiJianBASE index is restarted!", this.getClass().getName());
		}
		catch( Exception e ) {
	...
	public static void main(String[] args) {
		PeijianBaseCreateIndexTask ts = new PeijianBaseCreateIndexTask();
		ts.createIndex();
	}
}
