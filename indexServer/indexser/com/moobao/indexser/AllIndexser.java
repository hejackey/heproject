 ...
 * @author liuxueyong
 */
public class AllIndexser {
	
	IndexWriter writer = null;
	IndexReader reader = null;
	
	//得到写索引和修改索引对象
	public AllIndexser( IndexWriter writer ) {
		this.writer = writer;
	}
	
	//得到修改索引对象
	public AllIndexser( IndexReader reader) {
		this.reader = reader;
	}
    ...
    public void indexDel(Document doc) {
    	String productID = doc.get( "primaryId" );
    	try {
    		//删除Document
    		Term term = new Term( "primaryId", productID );
    ...
    public void indexDel(List<Document> listdoc) {
    	...
    	try {
    		//删除Document列表
    		for( int i = 0; i < listdoc.size(); i ++ ) {
    			productID = listdoc.get(i).get( "primaryId" );
    			term = new Term( "primaryId", productID );
    			reader.deleteDocuments( term );
    		}
    	}
    	catch( IOException e ) {
    ...
    public void indexDel(String keyid) {
    	try {
    		//删除Document
    		Term term = new Term( "primaryId", keyid );
        	reader.deleteDocuments(term);
    	}
    	catch( IOException e ) {
    		e.printStackTrace();
